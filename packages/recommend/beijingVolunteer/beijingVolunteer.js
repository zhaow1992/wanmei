...
Page({
    ...
    onShareAppMessage: function onShareAppMessage(res) {
        ...
        if (that.options && that.options.numId) {
            return {
                ...
                path: "/packages/recommend/beijingVolunteer/beijingVolunteer?numId=" + that.options.numId + "&share=true"
            };
        } else {}
    },
    //重新筛选院校列表
    ...
    getZyTable: function getZyTable() {
        var _this5 = this;
        var that = this;
        api.getBJZyTable("Users/ZyTable/BeiJing/Get", "POST", this.options.numId).then(function(res) {
            ...
            that.sensorsData = data;
            if (that.options.operationType) {
                that.InsertVoluntSheets(that.options.operationType);
            }
        });
    },
    InsertVoluntSheets: function InsertVoluntSheets(operationType) {
        var that = this;
        var data = that.sensorsData;
        var sensorsData = {
            operation_type: operationType,
            //操作类型
            sheets_num: data.sheets_num,
            //志愿表编号
            sheets_type: data.sheets_type,
            //志愿表类型
            sheets_source: data.sheets_source,
            //志愿表来源
            data_province: data.data_province,
            //所属省份
            ...
            reliance_rate: "",
            //志愿靠谱率
            colg_num: data.colg_num,
            //院校填报个数
            major_num: data.major_num
        };
        app.sensors.track("VoluntSheets", sensors.VoluntSheets(sensorsData));
    },
    // 初始化
    init: function init() {
    ...
    saveZyTable: function saveZyTable() {
        ...
        api.SaveBjTable("Users/ZyTable/BeiJing/Save", "POST", data).then(function(res) {
            // if (that.options && that.options.numId) {
            // } else {
