...
// packages/recommend//beijingRecommend/beijingRecommend.js
var app = getApp();
...
Page({
    ...
    InsertRisk: function InsertRisk(num) {
        ...
        var FilloutFilterData = {
            fillout_type: data.SA_fillout_type,
            //填报类型
            data_batch: data.SA_data_batch,
            //所属批次
            batch_filter: data.SA_batch_filter,
            //批次筛选
            majors_name: data.SA_majors_name,
            //专业名称
            college_name: data.SA_college_name,
            //院校名称
            score_rank_area: data.SA_score_rank_area,
            //位次区间
            province_filter: data.SA_province_filter,
            //所在地筛选
            tag_filter: data.SA_tag_filter,
            //标签筛选
            type_filter: data.SA_type_filter,
            //类型筛选
            nature_filter: data.SA_nature_filter,
            //性质筛选
            other_filter: data.SA_other_filter,
            //其它筛选
            results_number: data.SA_results_number
        };
    },
    //预览志愿表里点击院校显示删除按钮
    ...
    getZyCount: function getZyCount() {
        var _this = this;
        var that = this;
        _api4.default.getZyCount("TZY/Recommendation/GetZyCount", "POST", this.userInfo[0].Province, this.data.score.totalScore, this.userInfo[0].courseType, this.data.batch, "").then(function(res) {
            if (res.isSuccess) {
                var len = res.result.zyCollegeCount;
                var zyTableList = _this.data.zyTableList;
    ...
    onLoad: function onLoad(options) {
        ...
        var batch = wx.getStorageSync("bjBatch");
        if (batch) {
            this.setData({
                batch: batch
            });
    ...
    goSearch: function goSearch() {
        wx.navigateTo({
            url: "/pages/globalSearch/globalSearch?mode=bjReport"
        });
    },
    ...
    saveZyTable: function saveZyTable() {
        ...
        _api2.default.SaveBjTable("Users/ZyTable/BeiJing/Save", "POST", data).then(function(re) {
            wx.hideLoading();
            if (re.isSuccess) {
                wx.showModal({
                    title: "保存成功",
                    ...
                    success: function success(res) {
                        if (res.confirm) {
                            wx.navigateTo({
                                url: "../beijingVolunteer/beijingVolunteer?numId=" + re.result + "&operationType=生成"
                            });
                            setTimeout(function() {
                ...
                if (ele.collegeName == collegename && ele.chooseSubjects == that.chooseSubjects) {
                    ...
                    if (res) {
                        ...
                        if (ele.professions.length >= that.data.majorCount) {
                            ...
                            ele.professions.push(_this5.currentMajorInfo);
                            _this5.changeisTianBao("oneClick");
                        }
                    }
                    if (ele.professions.length == 0) {
                        ...
                        _this5.code1 = "";
                        _this5.code2 = "";
                        _this5.currentMajorInfo = "";
                        _this5.chooseSubjects = "";
                        _this5.changeisTianBao("changeTab");
                    }
                }
            });
            this.setData({
    ...
    changeisTianBao: function changeisTianBao(type) {
        ...
        if (type == "oneClick") {
            ...
            listArr.collegeList.forEach(function(ele) {
                if (ele.collegeName == _this6.currentMajorInfo.collegeName) {
