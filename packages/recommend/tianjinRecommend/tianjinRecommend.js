...
// packages/recommend//tianjinRecommend/tianjinRecommend.js
var app = getApp();
...
Page({
    ...
    onLoad: function onLoad(options) {
        ...
        var batch = wx.getStorageSync("tjBatch");
        if (batch) {
            this.setData({
                batch: batch
            });
        ...
        //     let score = wx.getStorageSync('userScore').total;
        //     this.setData({
        //       batch: score >= 435 ? 1 : 2
        //     })
        //   }
    ...
    loadCollegeList: function loadCollegeList() {
        ...
        var layer = "本科A";
        if (this.data.batch == 1 && this.data.section) {
            layer = "本科" + this.data.section;
        }
        var data = {
            provinceId: 835,
            layer: layer,
            ...
            if (res.isSuccess) {
                ...
                for (var i = 0, j = data.collegeList.length; i < j; i++) {
                    ...
                    for (var m = 0, _n = data.collegeList[i].professions.length; m < _n; m++) {
                        ...
                        for (var _j = 0, k = zyTableList.length; _j < k; _j++) {
                            if (zyTableList[_j].majorCode == data.collegeList[i].professions[m].majorCode && zyTableList[_j].collegeId == data.collegeList[i].collegeId) {
    ...
    collegeType: function collegeType(e) {
        ...
        if (index == 0) {
            ...
            for (var _i = 0, _j2 = collegeType.length; _i < _j2; _i++) {
    ...
    foreignType: function foreignType(e) {
        ...
        if (index == 0) {
            ...
            for (var _i2 = 0, _j3 = foreignType.length; _i2 < _j3; _i2++) {
    ...
    chongzhi: function chongzhi() {
        ...
        classifyList[0].st = true;
        for (var _i3 = 1, _j4 = classifyList.length; _i3 < _j4; _i3++) {
            classifyList[_i3].st = false;
        }
        hotType[0].st = true;
        for (var _i4 = 1, _j5 = hotType.length; _i4 < _j5; _i4++) {
            hotType[_i4].st = false;
        }
        collegeType[0].st = true;
        for (var _i5 = 1, _j6 = collegeType.length; _i5 < _j6; _i5++) {
            collegeType[_i5].st = false;
        }
        foreignType[0].st = true;
        for (var _i6 = 1, _j7 = foreignType.length; _i6 < _j7; _i6++) {
    ...
    queren: function queren(flag, flag1) {
        ...
        for (var _i7 = 1, _j8 = classifyList.length; _i7 < _j8; _i7++) {
            if (classifyList[_i7].st) {
                this.classify.push(classifyList[_i7].name);
            }
        }
        for (var _i8 = 1, _j9 = hotType.length; _i8 < _j9; _i8++) {
            if (hotType[_i8].st) {
    ...
    goSearch: function goSearch() {
        var section = "本科A";
        if (this.data.batch == 1 && this.data.section) {
            section = "本科" + this.data.section;
        }
        wx.navigateTo({
            url: "/pages/globalSearch/globalSearch?mode=tjReport&section=" + section
        });
    },
    ...
    goMajorDetail: function goMajorDetail(e) {
        ...
        if (majorcode.length == 4) {
            ...
            wx.navigateTo({
                url: "/packages/selectMajor/majorDetail/majorDetail?majorcode=" + majorcode + "&cityid=835"
            });
        }
    },
    // 保存志愿表
    saveZyTable: function saveZyTable() {
        ...
        var data = {
            ...
            isBen: true,
            //本科批true 专科批false
            abType: this.data.section ? "本科" + this.data.section + "段" : "本科A段",
            section: this.data.batch == 1 ? "本科" : "专科",
            colleges: newArrData
        };
        _api2.default.SaveBjTable("Users/ZyTable/TianJin/Save", "POST", data).then(function(re) {
            wx.hideLoading();
            if (re.isSuccess) {
                wx.showModal({
                    title: "保存成功",
                    ...
                    success: function success(res) {
                        if (res.confirm) {
                            wx.navigateTo({
                                url: "../tianjinVolunteer/tianjinVolunteer?numId=" + re.result
                            });
                            setTimeout(function() {
    ...
    changeBatch: function changeBatch() {
        ...
        wx.navigateTo({
            url: "/packages/common/batchList/batchList?batch=" + this.data.batch + "update=true&section=" + section
        });
    },
    ...
    clickMe: function clickMe(e) {
        ...
        //拿到学校信息
            ...
            if (that.data.currentCollegeName == ele.collegeName && that.chooseSubjects == ele.chooseSubjects) {
                ...
                tempObj.provinceNumId = ele.provinceId;
